import type { Request, Response } from "express";
import { AppError } from "../lib/errors";
import * as eventService from "../services/event.service";

function param(req: Request, key: string): string {
	const val = req.params[key];
	const first = Array.isArray(val) ? val[0] : val;
	if (!first)
		throw new AppError(`Missing route param: ${key}`, 400, "BAD_REQUEST");
	return first;
}

function requireUserId(req: Request): string {
	const userId = req.user?.id;
	if (!userId) throw new AppError("Unauthorized", 401, "UNAUTHORIZED");
	return userId;
}

export async function create(req: Request, res: Response) {
	const event = await eventService.createEvent(
		param(req, "orgId"),
		requireUserId(req),
		req.body,
	);
	res.status(201).json({ ok: true, data: event });
}

export async function getBySlug(req: Request, res: Response) {
	const event = await eventService.getEventBySlug(param(req, "slug"));
	res.json({ ok: true, data: event });
}

export async function list(req: Request, res: Response) {
	let query = (req as Request & { validatedQuery: unknown })
		.validatedQuery as Parameters<typeof eventService.listEvents>[0];

	const hasBearer = req.headers.authorization?.startsWith("Bearer ");
	if (!hasBearer) {
		query = {
			...query,
			status: "PUBLISHED",
			visibility: "PUBLIC",
		};
	}

	const result = await eventService.listEvents(query);
	res.json({ ok: true, ...result });
}

export async function listByOrg(req: Request, res: Response) {
	const events = await eventService.listOrgEvents(param(req, "orgId"));
	res.json({ ok: true, data: events });
}

export async function update(req: Request, res: Response) {
	const event = await eventService.updateEvent(
		param(req, "eventId"),
		requireUserId(req),
		req.body,
	);
	res.json({ ok: true, data: event });
}

export async function remove(req: Request, res: Response) {
	await eventService.deleteEvent(param(req, "eventId"), requireUserId(req));
	res.json({ ok: true });
}
